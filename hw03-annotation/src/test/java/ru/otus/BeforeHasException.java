package ru.otus;

import ru.otus.annotation.Test;
import ru.otus.annotation.After;
import ru.otus.annotation.Before;

public class BeforeHasException {
    @Before
    public void before() {
        System.out.println("Before method was started");

        throw new RuntimeException("Something went wrong in before method");
    }

    @Test
    public void firstTest() {
        System.out.println("First test was started");
    }

    @Test
    public void secondTest() {
        System.out.println("Second test was started");
        throw new RuntimeException("Something went wrong in test");
    }

    @Test
    public void thirdTest() {
        System.out.println("Third test was started");
    }

    @After
    public void after() {
        System.out.println("After method was started");
    }
}
